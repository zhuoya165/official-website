...
module.exports = MinimalClass.extend({
	...
	load_vimeo_player: function (open) {
		...
		if (!this.player) {
			var self = this;
			this.player = new VimeoPlayerPopper({ _id: this.vimeo_id, _autoopen: open, _trigger: this.arrow, _onClose: function () {
...
module.exports = MinimalClass.extend({
	...
	create: function () {
		...
		if (this.cards_element.length) {
			...
			this.dots_element = this.element.find('.js-dots');
			if (this.dots_element.length) {
				this.dots = new Dots({ element: this.dots_element, delegate: self, _onChange: function (pos, dir, fromUser) {
	...
	recalc: function () {
		var ww = $(window).width(),
		    ...
		    card_padding = Math.round(ww * (ww <= this.max_screen_width ? 0.14 : 0.05)),
	...
	resize: function (ww, wh) {
        ...
        this.cards_element.css({ height: 460})
...
module.exports = MinimalClass.extend({
	...
	load_vimeo_player: function (open) {
		...
		if (!this.player) {
			var self = this;
			this.player = new VimeoPlayerPopper({ _id: this.vimeo_id, _trigger: this.play, _autoopen: open, _onClose: function (player, e) {
					self.close();
				} });
		} else {
			this.player.open();
